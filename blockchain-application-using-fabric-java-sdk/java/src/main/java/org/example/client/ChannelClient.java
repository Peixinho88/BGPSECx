...
public class ChannelClient {
	...
	public Collection<ProposalResponse> sendTransactionProposal(TransactionProposalRequest request)
		...
		try{
			response = channel.sendTransactionProposal(request, channel.getPeers());
			for (ProposalResponse pres : response) {
				...
				if (stringResponse.charAt(0) == 'V') {
					Logger.getLogger(ChannelClient.class.getName()).log(Level.INFO,
							ConsoleColors.GREEN + stringResponse + ConsoleColors.RESET);
				} else {
					Logger.getLogger(ChannelClient.class.getName()).log(Level.INFO,
							ConsoleColors.RED + stringResponse + ConsoleColors.RESET);
				}
			}
			...
			e.printStackTrace();
			System.out.println("Transaction proposal failed misteriously...");
		}
	...
	public TransactionInfo queryByTransactionId(String txnId) throws ProposalException, InvalidArgumentException {
		// Logger.getLogger(ChannelClient.class.getName()).log(Level.INFO,
		// 		"Querying by transaction id " + txnId + " on channel " + channel.getName());
